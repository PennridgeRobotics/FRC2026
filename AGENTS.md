...
- Default to the current season (2026) unless specified otherwise
...
## FRC2026 repo-specific notes
- Entry flow is `Main` -> `Robot` -> `RobotContainer`; keep initialization and bindings centralized there.
- `Robot` publishes build metadata to `NetworkTables/Metadata` in sim and starts `DataLogManager` on the real robot.
- The main subsystems are `SwerveSubsystem`, `FuelSubsystem`, `ClimberSubsystem`, and `LightsSubsystem`; most are feature-flagged through `Constants.*_ENABLED`.
- `RobotContainer` wires default commands and operator bindings, and uses `LoggedNetworkInput.runAllPeriodic()` via `preSchedulerUpdate()` for all tuning topics.
- Prefer the repo’s dashboard wrappers (`LoggedNetworkBoolean`, `LoggedNetworkUnit`, `LoggedNetworkStruct`, `LoggedNetworkSendable`) instead of raw NetworkTables writes.
- Autos are built through `AutoManager` and custom `Path`/`FollowPath` logic; alliance flipping is handled there, not in button code.
- `SwerveSubsystem` is the integration hub for drive, odometry, vision, bump detection, and shooter-facing behaviors.
- Vision is split by role: `VisionManager` owns PhotonVision sim + pose updates, while Limelight orientation updates happen inside the swerve layer.
- The custom joystick path is `CommandJoystickController` -> `JoystickController`; use those wrappers when working on the alternate operator setup.
- `src/main/deploy/swerve/` holds the YAGSL config and `src/main/deploy/autos/` holds auto path assets; keep deploy assets there.
- `src/main/java/frc/robot/util/BuildConstants.java` is generated by `gversion` and should not be hand-edited.
- Repo style is enforced by `.editorconfig` + Spotless: UTF-8, 4-space indents, trim trailing whitespace, final newline.
- Null-safety is strict: new classes should be annotated `@NullMarked`, and nullable fields/returns must be explicit with `@Nullable`.
- `README.md` is the source of truth for driver/operator mappings, CAN IDs, and LED priority ordering; update it when controls change.
- Validation in CI is `./gradlew clean spotlessCheck` and `./gradlew clean build` on Java 17; there are currently no repo tests under `src/test`.
